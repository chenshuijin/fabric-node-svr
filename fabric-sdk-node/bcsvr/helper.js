...
function queryChaincode(org, version, value, t){
	...
	(err) => {
		...
		if (response_payloads) {
			...
			console.log('decode:',response_payloads[0].toString('utf-8'));
			return true;
		} else {
...
function execChaincode(org, version, username, password, fcn, args){
	var client = new hfc();
	var chain = client.newChain(e2e.channel);
	var orgName = ORGS[org].name;
	var targets = [];
	...
	return hfc.newDefaultKeyValueStore({
		...
		.then((store) => {
		client.setStateStore(store);
		return testUtil.getSubmitter(username, password, client, org);
	})
		.then((admin) => {
		the_user = admin;
		...
		.then((response_payloads) => {
		if (response_payloads) {
			console.log('response_payloads decode:',response_payloads[0].toString('utf-8'));
			console.log('response_payloads :', response_payloads);
			return response_payloads[0].toString('utf-8');
		} else {
			throw new Error('Failed to get response on query');
		}
	},(err) => {
...
module.exports.execChaincode = execChaincode;
